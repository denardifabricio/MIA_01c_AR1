...
class TaxiEnvCustom():
    def __init__(self, GRID_SIZE, PICKUP_LOCATIONS, DROPOFF_LOCATIONS, OBSTACLES, img_base_path=None):
    ...
    def step(self, action):
        ...
        en_pickup_o_dropoff = (self.taxi_row, self.taxi_col) in self.pickups or \
                            (self.taxi_row, self.taxi_col) in self.dropoffs
        ...
        if street_direction and not en_pickup_o_dropoff:
            if (action == 0 and street_direction != "down") or \
            (action == 1 and street_direction != "up") or \
            (action == 2 and street_direction != "right") or \
            (action == 3 and street_direction != "left"):
                reward -= 5  # penalización por entrar en contramano
        ...
        if (self.taxi_row, self.taxi_col) in self.dropoffs and not self.in_taxi:
            reward -= 5  # penaliza estacionar sobre destino sin pasajero
